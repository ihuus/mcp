...
Use `ihuus-geospatial` to convert any address or location to `lat` and `lon` before
calling any other tool. Never guess or hallucinate coordinates.

### 2. Select Tools Based on the Query
- **Full neighborhood report**: call all domain tools after geocoding.
- **Focused query** (e.g. "is this area walkable?"): call only the relevant tool(s).
- **School queries**: geocode → districts → school search → school detail as needed.
...
> **Fire risk is only available for California.** For non-CA locations the tool returns 0.
> Always communicate this limitation.
...
5. **Risk** — flood and fire (with CA-only caveat if applicable)
...
## Response Strategy
- Confirm the geocoded address at the top of every report.
- Score of 0 → explicitly say data is unavailable for that metric.
- Always include the API `description` — it contains the human-readable nuance beyond the score.
- For high-stakes decisions (buying a home, relocating), recommend consulting official FEMA maps, local fire authorities, and state education databases.
- Maintain a professional, insightful, and data-forward tone.
